...
import { getInstitutionCourses, insertInstitutionCourse, insertInstitutionStudent } from "../src/lib/product.ts";
...
test("writes enforce institution membership and role; reads and duplicate codes stay scoped", async () => {
  ...
  try {
    ...
    await insertInstitutionCourse(sql, "owner", course);
    ...
    await insertInstitutionStudent(sql, "issuer", student);
    ...
    assert.deepEqual(audit.rows.map((row) => row.action), ["course.created", "course.created", "student.created", "student.created"]);
  } finally {
    await db.close();
  }
});
